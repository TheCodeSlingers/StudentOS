...
export class StudentController {
  ...
  static getEnrolledStudents = asyncHandler(async (req: Request, res: Response) => {
    ...
    const students = await StudentService.getEnrolledStudents(batchId, page, limit);
    return ApiResponse.success(res, students.data, 200, students.meta);
  });

  static revokeEnrollment = asyncHandler(async (req: Request, res: Response) => {
    const batchId = req.params.batchId as string;
  ...
  static getStudentProfile = asyncHandler(async (req: Request, res: Response) => {
    const membershipId = req.params.membershipId as string;
    const profile = await StudentService.getStudentProfile(membershipId);
    return ApiResponse.success(res, profile);
  });

  static updateStudentProfile = asyncHandler(async (req: Request, res: Response) => {
    const membershipId = req.params.membershipId as string;
    const data = req.body;
